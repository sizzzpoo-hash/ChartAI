...
7.  **Identify Trend Reversals (Break of Structure / Change of Character):** You MUST look for the earliest objective signs of a trend change.
    *   In a downtrend (a series of lower lows and lower highs), the first time the price breaks above the most recent lower high, it creates a **Change of Character (CHoCH)**. This is the first signal that the trend may be reversing from bearish to bullish.
    *   In an uptrend (a series of higher highs and higher lows), the first time the price breaks below the most recent higher low, it is a **Change of Character (CHoCH)**. This is the first signal that the trend may be reversing from bullish to bearish.
    *   After a CHoCH, a **Break of Structure (BoS)** in the new direction (e.g., a new higher high after a bullish CHoCH) confirms the new trend.
    *   You must state if you see a recent CHoCH or BoS, as this is a powerful signal for a new entry in the direction of the emerging trend.
8.  **Analyze the Primary Chart (For Entry):** Now focus on the primary chart ({{{chartDataUri}}}). In the context of the established macro trend, look for specific entry signals.
...
9.  **Analyze Indicators for Divergence & Confluence:** Using the provided indicator data, look for divergences between price and momentum oscillators (RSI, MACD). A divergence is a powerful leading indicator of a potential trend reversal.
...
10. **Analyze Volume:** Examine the volume data from the OHLCV payload.
    ...
    *   **High-Volume Nodes:** Scan the OHLCV data to identify individual candles with exceptionally high volume (e.g., more than double the average of the last 20 candles). The price range (high to low) of these specific "high-volume nodes" acts as a powerful support/resistance zone because a significant number of transactions occurred there. Note any high-volume nodes and treat them as key levels in your analysis.
