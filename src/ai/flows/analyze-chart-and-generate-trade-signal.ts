...
6.  **Analyze the Primary Chart (For Entry):** Now focus on the primary chart ({{{chartDataUri}}}). In the context of the established macro trend, look for specific entry signals. Analyze its candlestick patterns (e.g., engulfing, doji, hammer), momentum (using RSI and MACD from the provided data), and its position relative to the key levels and Smart Money zones identified. Ensure any potential signal aligns with the directional bias from step 1.
7.  **Analyze Volume & Liquidity:** Examine the volume data from the OHLCV payload again. Does volume confirm the specific price action for your entry signal? For a breakout, is volume increasing? During consolidation, is volume low? On a reversal candle, is there a spike in volume? A lack of confirming volume WEAKENS any signal.
8.  **Analyze Volatility (Bollinger Bands):** If Bollinger Bands data is provided, analyze the bands. Are they expanding (high volatility) or contracting (low volatility)? Is the price touching the upper or lower band, suggesting an overbought or oversold condition? This helps refine entry and exit points.
{{#if economicEvents}}
9.  **Check for Major Economic Events:** Review the upcoming economic events. If there is a high-impact event within the next 48 hours, you MUST mention it and explain that it is advisable to wait until after the event to enter any trade, as volatility can be unpredictable. This overrides any technical signal.
    - Upcoming Events: {{{economicEvents.eventSummary}}}
{{/if}}
{{#if fundamentalAnalysis}}
10. **Consider Fundamental Context:** Review the provided fundamental analysis. Does it support or contradict the technical picture? Note how this influences your bias, especially the market sentiment.
